package com.scurab.web.drifmaps.language;

/**
 * Interface to represent the constants contained in resource bundle:
 * 	'F:/0Work/eclipse-workspace-j2ee/DrifMaps/src/com/scurab/web/drifmaps/language/Words.properties'.
 */
public interface Words extends com.google.gwt.i18n.client.Constants {
  
  /**
   * Translated "Add".
   * 
   * @return translated "Add"
   */
  @DefaultStringValue("Add")
  @Key("Add")
  String Add();

  /**
   * Translated "AddBeer".
   * 
   * @return translated "AddBeer"
   */
  @DefaultStringValue("AddBeer")
  @Key("AddBeer")
  String AddBeer();

  /**
   * Translated "AddCon".
   * 
   * @return translated "AddCon"
   */
  @DefaultStringValue("AddCon")
  @Key("AddCon")
  String AddCon();

  /**
   * Translated "AddDetails".
   * 
   * @return translated "AddDetails"
   */
  @DefaultStringValue("AddDetails")
  @Key("AddDetail")
  String AddDetail();

  /**
   * Translated "AddNewItem".
   * 
   * @return translated "AddNewItem"
   */
  @DefaultStringValue("AddNewItem")
  @Key("AddNewItem")
  String AddNewItem();

  /**
   * Translated "AddPro".
   * 
   * @return translated "AddPro"
   */
  @DefaultStringValue("AddPro")
  @Key("AddPro")
  String AddPro();

  /**
   * Translated "Author".
   * 
   * @return translated "Author"
   */
  @DefaultStringValue("Author")
  @Key("Author")
  String Author();

  /**
   * Translated "Beers".
   * 
   * @return translated "Beers"
   */
  @DefaultStringValue("Beers")
  @Key("Beers")
  String Beers();

  /**
   * Translated "Cancel".
   * 
   * @return translated "Cancel"
   */
  @DefaultStringValue("Cancel")
  @Key("Cancel")
  String Cancel();

  /**
   * Translated "City".
   * 
   * @return translated "City"
   */
  @DefaultStringValue("City")
  @Key("City")
  String City();

  /**
   * Translated "Cons".
   * 
   * @return translated "Cons"
   */
  @DefaultStringValue("Cons")
  @Key("Cons")
  String Cons();

  /**
   * Translated "Contact".
   * 
   * @return translated "Contact"
   */
  @DefaultStringValue("Contact")
  @Key("Contact")
  String Contact();

  /**
   * Translated "Coordinates".
   * 
   * @return translated "Coordinates"
   */
  @DefaultStringValue("Coordinates")
  @Key("Coordinates")
  String Coordinates();

  /**
   * Translated "Country".
   * 
   * @return translated "Country"
   */
  @DefaultStringValue("Country")
  @Key("Country")
  String Country();

  /**
   * Translated "Delete".
   * 
   * @return translated "Delete"
   */
  @DefaultStringValue("Delete")
  @Key("Delete")
  String Delete();

  /**
   * Translated "Deleting".
   * 
   * @return translated "Deleting"
   */
  @DefaultStringValue("Deleting")
  @Key("Deleting")
  String Deleting();

  /**
   * Translated "Detail".
   * 
   * @return translated "Detail"
   */
  @DefaultStringValue("Detail")
  @Key("Detail")
  String Detail();

  /**
   * Translated "Edit".
   * 
   * @return translated "Edit"
   */
  @DefaultStringValue("Edit")
  @Key("Edit")
  String Edit();

  /**
   * Translated "Hide".
   * 
   * @return translated "Hide"
   */
  @DefaultStringValue("Hide")
  @Key("Hide")
  String Hide();

  /**
   * Translated "Hide streetview".
   * 
   * @return translated "Hide streetview"
   */
  @DefaultStringValue("Hide streetview")
  @Key("HideStreetView")
  String HideStreetView();

  /**
   * Translated "ID".
   * 
   * @return translated "ID"
   */
  @DefaultStringValue("ID")
  @Key("ID")
  String ID();

  /**
   * Translated "X".
   * 
   * @return translated "X"
   */
  @DefaultStringValue("X")
  @Key("LatLngX")
  String LatLngX();

  /**
   * Translated "Y".
   * 
   * @return translated "Y"
   */
  @DefaultStringValue("Y")
  @Key("LatLngY")
  String LatLngY();

  /**
   * Translated "Name".
   * 
   * @return translated "Name"
   */
  @DefaultStringValue("Name")
  @Key("Name")
  String Name();

  /**
   * Translated "No".
   * 
   * @return translated "No"
   */
  @DefaultStringValue("No")
  @Key("No")
  String No();

  /**
   * Translated "Note".
   * 
   * @return translated "Note"
   */
  @DefaultStringValue("Note")
  @Key("Note")
  String Note();

  /**
   * Translated "OK".
   * 
   * @return translated "OK"
   */
  @DefaultStringValue("OK")
  @Key("OK")
  String OK();

  /**
   * Translated "Patio".
   * 
   * @return translated "Patio"
   */
  @DefaultStringValue("Patio")
  @Key("Patio")
  String Patio();

  /**
   * Translated "Pros".
   * 
   * @return translated "Pros"
   */
  @DefaultStringValue("Pros")
  @Key("Pros")
  String Pros();

  /**
   * Translated "Rating".
   * 
   * @return translated "Rating"
   */
  @DefaultStringValue("Rating")
  @Key("Rating")
  String Rating();

  /**
   * Translated "Really ?".
   * 
   * @return translated "Really ?"
   */
  @DefaultStringValue("Really ?")
  @Key("ReallyQstn")
  String ReallyQstn();

  /**
   * Translated "Save".
   * 
   * @return translated "Save"
   */
  @DefaultStringValue("Save")
  @Key("Save")
  String Save();

  /**
   * Translated "Saving".
   * 
   * @return translated "Saving"
   */
  @DefaultStringValue("Saving")
  @Key("Saving")
  String Saving();

  /**
   * Translated "Search".
   * 
   * @return translated "Search"
   */
  @DefaultStringValue("Search")
  @Key("Search")
  String Search();

  /**
   * Translated "Star".
   * 
   * @return translated "Star"
   */
  @DefaultStringValue("Star")
  @Key("Star")
  String Star();

  /**
   * Translated "Street".
   * 
   * @return translated "Street"
   */
  @DefaultStringValue("Street")
  @Key("Street")
  String Street();

  /**
   * Translated "Type".
   * 
   * @return translated "Type"
   */
  @DefaultStringValue("Type")
  @Key("Type")
  String Type();

  /**
   * Translated "WebLink".
   * 
   * @return translated "WebLink"
   */
  @DefaultStringValue("WebLink")
  @Key("WebLink")
  String WebLink();

  /**
   * Translated "Yes".
   * 
   * @return translated "Yes"
   */
  @DefaultStringValue("Yes")
  @Key("Yes")
  String Yes();
}
